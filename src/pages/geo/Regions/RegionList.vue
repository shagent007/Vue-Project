...
import { RegionDto } from "@/api/clients.ts";
...
export default class RegionList extends Vue {
  ...
  async itemsProvider() {
    try {
      return await applicationFacade.regions.getAll();
    } catch (e) {
      return [];
    }
  }

  async remove(item: RegionDto) {
    ...
    await applicationFacade.regions.remove(item.id);
    this.table.refresh();
  }
