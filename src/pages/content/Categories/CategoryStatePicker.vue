...
import { ContentCategoryState } from "@/api/clients.ts";

@Component
export default class CategoryStatesPicker extends Vue {
  @Prop({ required: true, type: Array }) value!: Array<ContentCategoryState>;
  ...
  set publish(value: boolean) {
    value ? this.value.push(1) : this.value.splice(this.value.indexOf(1), 1);
  }
  ...
  set draft(value: boolean) {
    value ? this.value.push(0) : this.value.splice(this.value.indexOf(0), 1);
  }
  ...
  set removed(value: boolean) {
    value ? this.value.push(2) : this.value.splice(this.value.indexOf(2), 1);
  }
}
</script>
