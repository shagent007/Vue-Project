...
import { CityDto } from "@/api/clients.ts";
...
export default class CityList extends Vue {
    ...
    async itemsProvider() {
        try {
            return await applicationFacade.cities.getAll(null);
        } catch (e) {
            return [];
        }
    }
    ...
    mounted() {
        const data = this.itemsProvider();
        console.log(data);
    }

    async remove(item: CityDto) {
        ...
        await applicationFacade.cities.remove(item.id);
        this.table.refresh();
    }
