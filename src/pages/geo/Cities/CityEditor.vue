          ...
          <div class="modal-body">
            ...
            <form class>
              <div class="position-relative row form-group">
                <label for="sku" class="col-sm-2 col-form-label">Регион</label>
                <div class="col-sm-10">
                  <region-select v-model="model"></region-select>
                </div>
              </div>
...
import { CityStateDto, ValidationErrorDto } from "@/api/clients";
import applicationFacade from "@/api/applicationFacade";
import { Prop } from "vue-property-decorator";
import Error from "@/pages/common/Error.vue";
import RegionSelect from "@/pages/geo/common/RegionSelect.vue";
...
export default class CityEditor extends DialogComponent<boolean> {
  @Prop({ required: true, type: Object }) readonly model!: CityStateDto;
  ...
  async submit() {
    try {
      await applicationFacade.cities.save(this.model);
      this.$close(true);
    } catch (er) {
