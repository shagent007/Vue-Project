...
import { OptionDto } from "@/api/clients.ts";
...
export default class OptionList extends Vue {
  ...
  async itemsProvider() {
    try {
      return await applicationFacade.optionsService.getAll(this.projectTypeId);
    } catch (e) {
      return [];
    }
  }
  ...
  async remove(item: OptionDto) {
    const result = await confirmRemove(
      "Внимание",
      "Вы действительно хотите удалить данную опцию?"
    );
    if (!result) return;

    await applicationFacade.optionsService.remove(item.id);
    this.table.refresh();
  }
