...
export default class CategoryGroupEditor extends DialogComponent<boolean> {
  @Prop({ required: true, type: Object })
  readonly model!: CategorySchemaUiGroupStateDto;
  @Prop({ required: true }) readonly schema!: CategorySchemaDto;

  errors: ValidationErrorDto[] = [];

  get title() {
    return !this.model.id ? "Новая ui группа" : "Редактирование ui группы";
  }
  ...
  async submit() {
    try {
      await applicationFacade.categories.saveUiGroup(
        this.schema.id,
        this.model
      );
      this.$close(true);
