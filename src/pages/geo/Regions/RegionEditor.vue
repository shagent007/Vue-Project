          ...
          <div class="modal-body">
            ...
            <form class>
              <div class="position-relative row form-group">
                <label for="sku" class="col-sm-2 col-form-label">Округ</label>
                <div class="col-sm-10">
                  <area-select v-model="model"></area-select>
                </div>
              </div>
...
import { RegionStateDto, ValidationErrorDto } from "@/api/clients";
import applicationFacade from "@/api/applicationFacade";
import { Prop, Ref } from "vue-property-decorator";
import { BModal } from "bootstrap-vue";
import Error from "@/pages/common/Error.vue";
import AreaSelect from "@/pages/geo/common/AreaSelect.vue";
...
export default class RegionEditor extends DialogComponent<boolean> {
  @Prop({ required: true, type: Object }) readonly model!: RegionStateDto;

  errors: ValidationErrorDto[] = [];

  get title() {
    return !this.model.id ? "Новый регион" : "Редактирование региона";
  }

  async submit() {
    try {
      await applicationFacade.regions.save(this.model);
      this.$close(true);
    } catch (er) {
